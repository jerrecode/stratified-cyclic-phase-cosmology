...
The initial implementation is a scientifically conservative baseline. It contains standard FLRW comparison models, a canonical scalar stratification field in FLRW spacetime, constraint diagnostics, reproducible model-comparison grids, a machine-validated public-data manifest, and a modular LaTeX paper. It does **not** claim that a stable cycle, a fundamental discrete time, or a physical vortex has already been demonstrated.
...
Every run should be driven by a committed YAML configuration and should emit a provenance record containing the Git commit, dependency versions, configuration hash, solver settings, platform information, and output checksums. See `docs/reproducibility.md`.
